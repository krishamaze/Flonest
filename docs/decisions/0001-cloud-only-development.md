...
### Alternative 1: Local Supabase with Docker
- **Pros**: Offline development, faster local queries, no cloud costs during dev
- **Cons**: Docker complexity, migration sync issues, maintenance overhead
- **Why rejected**: Complexity outweighed benefits; git-based migrations conflicted with local-first approach

### Alternative 2: Dual Mode (Local + Cloud)
- **Pros**: Flexibility to choose based on context
- **Cons**: Increased complexity, confusion about which mode to use, harder to standardize
- **Why rejected**: Too much overhead to support both modes reliably

### Alternative 3: Local PostgreSQL (No Supabase)
