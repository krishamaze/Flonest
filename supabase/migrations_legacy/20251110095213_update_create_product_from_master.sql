-- Migration: Update create_product_from_master RPC
-- Adds approval_status check to ensure only approved master products can be used
...
BEGIN
  ...
  -- Fetch master product details - must be approved and active
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Master product not found, inactive, or not approved';
  END IF;
