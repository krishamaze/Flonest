-- Migration: Update auto_link_product_to_master RPC
-- Updates to create pending master products with governance fields
...
BEGIN
  ...
  -- Create new master product from org product data with approval_status='pending'
  ...
  VALUES (
    ...
    v_product.category,
    NULL, -- HSN code not available, will be set on approval
    ...
    'pending', -- Approval status: pending until internal review
    v_user_id, -- Created by user who triggered auto-link
    p_org_id -- Submitted by org
  )
  ...
  VALUES (
    ...
    NOW(),
    'Master product auto-created from org product during invoice creation',
    NULL,
    'pending'
  );
...
GRANT EXECUTE ON FUNCTION auto_link_product_to_master(uuid, uuid, uuid) TO authenticated;
