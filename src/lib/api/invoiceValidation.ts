 ...
 * Platform admins only - queries invoices with products that aren't approved or missing HSN
 *
 * Note: For platform admins, RLS policy allows SELECT on all invoices regardless of org_id filter.
 * The org_id filter only applies to non-platform-admin users, but this function is only called from
 * platform admin pages, so platform admins will see all blocked invoices across all orgs.
 */
export async function getBlockedInvoices(
  ...
  // RLS policy invoices_read_internal allows platform admins to SELECT all invoices
  // Org filter is ignored for platform admins via RLS, but can be used for filtering if needed
  ...
  // Apply org filter if provided (for non-platform-admin users or specific org filtering)
  // Platform admins will see all invoices regardless due to RLS policy
  if (filters?.org_id) {
